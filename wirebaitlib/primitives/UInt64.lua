...
local uint_64 = {}
uint_64.__index = uint_64;


--------------------------------------------------- metamethods --------------------------------------------------------
function uint_64:__tostring()
    return self.m_decimal_value_str;
end
...
function uint_64.__add(uint_or_num1, uint_or_num2)
    ...
    return UInt64Class.new(new_low_word, new_high_word);
end
...
function uint_64.__sub(uint_or_num1, uint_or_num2)
    ...
    return UInt64Class.new(low_word1, high_word1) + UInt64Class.new(low_word2, high_word2);
end
...
function uint_64.__band(num_or_uint1, num_or_uint2) --[[bitwise AND operator (&)]]
    ...
    return UInt64Class.new(bw.And(low_word1, low_word2), bw.And(high_word1, high_word2))
end

function uint_64:__bnot() --[[bitwise NOT operator (unary ~)]]
    return UInt64Class.new(bw.And(bw.Not(self.m_low_word), WORD_MASK), bw.And(bw.Not(self.m_high_word), WORD_MASK))
end

function uint_64.__bor(uint_or_num1, uint_or_num2) --[[bitwise OR operator (|)]]
    local low_word1, high_word1 = getWords(uint_or_num1);
    local low_word2, high_word2 = getWords(uint_or_num2);
    return UInt64Class.new(bw.Or(low_word1, low_word2), bw.Or(high_word1, high_word2))
end

function uint_64.__bxor(uint_or_num1, uint_or_num2) --[[bitwise XOR operator (binary ~)]]
    local low_word1, high_word1 = getWords(uint_or_num1);
    local low_word2, high_word2 = getWords(uint_or_num2);
    return UInt64Class.new(bw.Xor(low_word1, low_word2), bw.Xor(high_word1, high_word2))
end
...
function uint_64:__shl(shift) --[[bitwise left shift (<<)]]
    ...
    if shift < 32 then
        ...
        return UInt64Class.new(bw.And(bw.Lshift(self.m_low_word, shift), WORD_MASK), new_high_word);
    elseif shift < 64 then
        return UInt64Class.new(0, bw.And(bw.Lshift(self.m_low_word, (shift-32)), WORD_MASK));
    else
        return UInt64Class.new(0, 0);
    end
end
...
function uint_64:__shr(shift) --[[bitwise right shift (>>)]]
    ...
    if shift < 32 then
        ...
        return UInt64Class.new(new_low_word, bw.Rshift(self.m_high_word, shift));
    elseif shift < 64 then
        return UInt64Class.new(bw.And(bw.Lshift(self.m_high_word, (shift-32)), WORD_MASK), 0);
    else
        return UInt64Class.new(0, 0);
    end
end
...
---------------------------------------------------- Constructor -------------------------------------------------------
function UInt64Class.new(num, high_num)
    assert(num and type(num) == "number" and num == math.floor(num) and num >= 0 and num <= UINT32_MAX,
            "UInt64.new(num), num must be a positive 32 bit integer!");
    assert(not high_num or (type(high_num) == "number" and high_num == math.floor(high_num) and high_num >= 0 and high_num <= UINT32_MAX),
            "UInt64.new(num, high_num): when provided, high_num must be a positive 32 bit integer!");
    ...
    new_uint64.m_decimal_value_str = decimalStrFromWords(new_uint64.m_low_word, new_uint64.m_high_word);

    -- Setting metatable
    setmetatable(new_uint64, uint_64)
    return new_uint64;
end
...
function UInt64Class.fromByteArray(byte_array)
    ...
    return UInt64Class.new(num, high_num);
end

--[[Returns a UInt64 instance of maximum value]]
function UInt64Class.max()
    return UInt64Class.new(UINT32_MAX, UINT32_MAX);
end

--[[Returns a UInt64 instance of value 0]]
function UInt64Class.min()
    return UInt64Class.new(0, 0);
end
