...
if disable_lua == nil and not _WIREBAIT_ON_ then  --disable_lua == nil checks if this script is being run from wireshark.
  ...
  local dissector_tester = wirebait.plugin_tester.new({dissector_filepath="example/demo_dissector.lua", only_show_dissected_packets=true});
  ...
  .. "00 00 00 01 57 69 72 65   62 61 69 74 00 62 79 20" 
  .. "4D 61 72 6B 6F 50 61 75   6C 30 00 00 AA BB CC 11" 
  .. "22 33 C0 A8 0E 1C AB CD   EF 12 34 56 78 90 AB CD" 
  .. "EF 12 34 56 78 90 00 00   00 00 00 00 00 00 00 00" 
  ...
  dissector_tester:dissectPcap("example/captures/demo.pcap")           --dissetion from packet data contained in a pcap file
