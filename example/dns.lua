...
if disable_lua == nil and not _WIREBAIT_ON_ then  --disable_lua == nil checks if this script is being run from wireshark.
    ...
    local dissector_tester = wirebait.new({dissector_filepath="example/dns.lua", only_show_dissected_packets=true});
    ...
    dissector_tester:dissectPcap("/Users/markus/Desktop/dns_sample.pcap")
...
function dns.dissector(tvbuf,pktinfo,root)
    ...
    if isResponse() then
         --[[TODO]]--query_flag_tree:add_proto_expert_info(ef_response, "It's a response!")
        if transid == 42 then
    ...
    --[[TODO]]--tree:add(pf_num_additional_rr, tvbuf:range(10,2):range()())
...
DissectorTable.get('udp.port'):add(5740, dns)
